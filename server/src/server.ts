...
async function handleMessage(senderId: string, msg: MessagingEvent['message']) {
  ...
  // When user clicks quick reply buttons, use the payload instead of text
  let userInput = text.toLowerCase();
  if (msg.quick_reply?.payload) {
    userInput = msg.quick_reply.payload.toLowerCase();
    console.log(`📱 Quick reply clicked: ${userInput}`);
  }
  ...
  // Cancel command
  if (userInput.includes("cancel") || userInput.includes("reset")) {
    clearState(senderId);
    await sendTypingIndicator(senderId, false);
    return callSendAPI(senderId, {
      text: `🔄 Request cancelled. Type **HELP** when you need assistance.`
  ...
  if (currentState.state === "awaiting_assistance_type") {
    ...
    if (validTypes.includes(userInput)) {
      updateEmergencyData(senderId, { 
        requiredAssistance: userInput.charAt(0).toUpperCase() + userInput.slice(1) 
      });
      setState(senderId, "awaiting_contact_name");
      await sendTypingIndicator(senderId, false);
      return callSendAPI(senderId, {
        text: `✅ Required Assistance: **${userInput.toUpperCase()}**\n\n` +
              `👤 **Contact Name**\n` +
              `Please provide your full name:`
      });
    } else {
      await sendTypingIndicator(senderId, false);
      return callSendAPI(senderId, {
        text: `⚠️ Please select a valid option:\n\nFood, Water, Medical, Shelter, Clothing, or Other`
      });
    }
  }
  ...
  if (currentState.state === "awaiting_urgency_level") {
    ...
    if (validLevels.includes(userInput)) {
      updateEmergencyData(senderId, { 
        urgencyLevel: userInput.charAt(0).toUpperCase() + userInput.slice(1) 
      });
      setState(senderId, "awaiting_location");
      await sendTypingIndicator(senderId, false);
      return askForLocation(senderId);
      ...
      await sendTypingIndicator(senderId, false);
      return callSendAPI(senderId, {
        text: `⚠️ Please select a valid urgency level:\n\nLow, Medium, High, or Critical`
      });
    }
  }
  ...
  // STATE: Awaiting additional info
  if (currentState.state === "awaiting_additional_info") {
    if (userInput === "skip" || userInput === "done") {
      // Submit the request
      await submitEmergencyRequest(senderId);
      return;
    }
